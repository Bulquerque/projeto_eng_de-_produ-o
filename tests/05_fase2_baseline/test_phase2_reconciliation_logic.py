...
for (const companyId of ['empresa1','empresa2']) {
  ...
  if (companyId === 'empresa1') {
    ...
    if (reconciliation.tax.status !== 'aligned') throw new Error('empresa2 tax should be aligned');
    if (reconciliation.tax.raw_status !== 'within_tolerance') throw new Error('empresa2 raw tax status should be preserved');
    if (reconciliation.overall.status !== 'aligned') throw new Error('empresa2 overall should be aligned');
  }
}
