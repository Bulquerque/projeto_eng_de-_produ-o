...
def test_phase_folders_exist_with_module_docs():
    contracts = json.loads((ROOT / 'data/contracts/module-contracts.json').read_text(encoding='utf-8'))
    assert (ROOT / 'phases/README.md').exists()
    for slug in PHASES:
        p = ROOT / 'phases' / slug
        assert p.exists()
        assert (p / 'README.md').exists()
        assert (p / 'modules').exists()
...
if __name__ == '__main__':
    test_phase_folders_exist_with_module_docs()
    print('PHASE_FOLDER_MODULE_DOCS_OK')
