...
export function normalizeTaxRegimeId(value) {
...
function normalizeAliasMap(map = {}) {
  return Object.fromEntries(
    Object.entries(map).map(([key, value]) => [
      normalizeTaxRegimeId(key),
      normalizeTaxRegimeId(value),
    ])
  );
}
...
function normalizeRegimes(regimes = {}) {
  ...
  for (const [regimeId, regime] of Object.entries(regimes)) {
    const normalizedId = normalizeTaxRegimeId(regimeId);
    const normalizedRegime = clone(regime || {});
    if (normalizedRegime.current_weight == null) {
      const weightEntry = Object.entries(normalizedRegime).find(
        ([key]) => key.endsWith('_weight') && key !== 'reform_weight'
      );
      if (weightEntry) normalizedRegime.current_weight = weightEntry[1];
    }
    out[normalizedId] = {
      ...normalizedRegime,
      regime_id: normalizedId,
    };
  }
  return out;
}
...
export function normalizeTaxReformConfig(input = {}) {
  ...
  merged.default_tax_regime = normalizeTaxRegimeId(merged.default_tax_regime);
...
export function resolveTaxRegime({
  ...
  const normalizedRegime = normalizeTaxRegimeId(taxRegime);
  const canonicalMode = normalizeTaxRegimeId(normalizedMode);
  if (normalizedRegime && config.regimes[normalizedRegime]) return normalizedRegime;
  if (canonicalMode && config.regimes[canonicalMode]) return canonicalMode;
  ...
  return config.default_tax_regime || 'current';
}

export function resolveTaxModeForRegime(regimeId, config = getTaxReformConfig()) {
  const regime = config.regimes[normalizeTaxRegimeId(regimeId)] || null;
  ...
  return regime.regime_id === 'current' ? 'current' : regime.regime_id;
}

export function taxRegimeLabel(regimeId, config = getTaxReformConfig()) {
  const regime = config.regimes[normalizeTaxRegimeId(regimeId)] || null;
  return regime?.label || String(regimeId || config.default_tax_regime || 'current');
}
