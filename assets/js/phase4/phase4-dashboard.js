...
function currentWeights() {
  ...
  const w = {};
  ids.forEach((id) => (w[id] = Number($(`weight_${id}`)?.value || 0) / 100));
  return w;
}
function setWeights(weights) {
  for (const [k, v] of Object.entries(weights || {})) {
    const el = $(`weight_${k}`);
    if (el) el.value = Math.round(Number(v) * 100);
  }
  renderObjectivePreview();
}
function renderObjectivePreview() {
  state.objective = buildObjective({
...
function renderOptimizerInputTable() {
  const el = $('optimizerInputTable');
  if (el)
    el.innerHTML = buildOptimizerInputTableHtml({
      bundle: state.bundle,
      companyId: state.companyId,
...
function renderProfiles() {
  const profiles = loadDefaultProfiles();
  $('objectiveProfiles').innerHTML = profiles
    .map(
      (p) =>
        `<button type="button" class="profile-card" data-profile="${p.profile_id}"><strong>${escapeHtml(p.profile_name)}</strong><span>${escapeHtml(p.description)}</span></button>`
    )
    .join('');
}
function renderOptimizer() {
  ...
  const exp = explainRanking({ objective: state.objective, scoredScenario: opt.best_scenarios[0] });
  $('rankingExplanation').innerHTML =
    `<ul>${exp.ranking_explanation.map((x) => `<li>${escapeHtml(x)}</li>`).join('')}</ul>`;
  state.frontier = buildTradeoffFrontier({
    companyId: state.companyId,
    scenarioRecords: opt.scenario_records,
    scoredScenarios: opt.scored_scenarios,
  });
  $('tradeoffPanel').innerHTML = buildTradeoffTableHtml(state.frontier);
  renderRankingChart(opt.best_scenarios);
  renderTradeoffChart(state.frontier);
}
function constraints() {
  return {
    min_active_cds: Number($('minActiveCds').value || 1),
...
function runOpt() {
  renderObjectivePreview();
  if (!state.objective.valid) {
    alert('Objetivo inválido. Corrija os pesos.');
    return;
  }
...
export async function loadPhase4Company(companyId) {
  if (state.isLoading) {
    log('loadPhase4Company ignorado — carregamento em andamento', { companyId });
    return;
  }
  ...
  try {
    ...
    log('Empresa carregada', { companyId });
  } catch (e) {
    if (loading)
      loading.innerHTML = `<div class="alert-box error"><strong>Erro</strong><p>${escapeHtml(e.message)}</p></div>`;
...
export function setupPhase4() {
  renderProfiles();
  document
    .querySelectorAll('[data-company]')
    .forEach((btn) => btn.addEventListener('click', () => loadPhase4Company(btn.dataset.company)));
  document.body.addEventListener('click', (e) => {
    const prof = e.target.closest('[data-profile]');
    if (prof) {
      const obj = cloneProfileAsObjective(prof.dataset.profile, state.companyId);
      $('objectiveName').value = obj.objective_name;
  ...
  [
    'objectiveName',
    'weight_total_cost',
    'weight_service_quality',
    'weight_operational_risk',
    'weight_tax_impact',
    'weight_inventory_efficiency',
  ].forEach((id) => $(id)?.addEventListener('input', renderObjectivePreview));
  [
    'minActiveCds',
    'maxActiveCds',
    'maxCdShare',
