...
export function renderShell({ companyId, route, debugEnabled = false } = {}) {
  const companies = Object.values(COMPANY_REGISTRY)
    ...
    .join('');
  return `<div id="networkAppRoot" class="network-app" data-runtime="network-intelligence">
    <aside class="network-sidebar" aria-label="Navegação Network Intelligence">
      <div class="network-brand"><span class="network-brand-mark">V</span><div><strong>Network</strong><small>Intelligence</small></div></div>
      <nav class="network-nav" aria-label="Seções principais">
        <a href="#/network/overview/summary" data-route="#/network/overview/summary" data-section="overview">Visão executiva</a>
        <a href="#/network/scenarios/build" data-route="#/network/scenarios/build" data-section="scenarios">Cenários</a>
        <a href="#/network/optimizer/configure" data-route="#/network/optimizer/configure" data-section="optimizer">Otimizador</a>
        <a href="#/network/trust/overview" data-route="#/network/trust/overview" data-section="trust">Dados & confiança</a>
        ${debugEnabled ? '<a href="#/network/dev/console" data-route="#/network/dev/console" data-section="dev">Debug</a>' : ''}
      </nav>
      <div class="network-sidebar-foot"><small>Build modular 6.2</small><span data-testid="mode-badge">runtime</span></div>
    </aside>
    <div class="network-content">
      <header class="network-topbar">
        ...
        <div class="network-topbar-actions">
          ...
          <button type="button" class="ni-button secondary" data-action="open-dev" data-testid="dev-console">Dev</button>
        </div>
      </header>
      <main id="networkPage" class="network-page" data-route-current="${escapeHtml(route || '')}" tabindex="-1" aria-live="polite"></main>
    </div>
    <div id="networkDrawerBackdrop" class="network-drawer-backdrop" hidden></div>
    <aside id="networkDrawer" class="network-drawer" role="dialog" aria-modal="true" aria-labelledby="networkDrawerTitle" hidden>
      ...
      <h2 id="networkDrawerTitle">Detalhes</h2><div id="networkDrawerBody"></div>
    </aside>
...
export function updateGlobalContext(root, state) {
  ...
  if (companyBadge) companyBadge.textContent = state.context.company_id || '—';
  if (scenarioSelect) {
