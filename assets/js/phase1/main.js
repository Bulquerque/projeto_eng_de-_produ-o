...
import {
  SHARED_DEBUG_FEED_KEY,
  appendSharedDebugEntry,
  readSharedDebugEntries,
} from '../core/debug-tools.js';
import { canUseStorage, readStorageJSON, writeStorageJSON } from '../core/browser-storage.js';
import {
  fetchJson,
  fetchText,
  loadEncryptedManifest,
  loadPhase2Bundle,
} from '../core/data-loader.js';
import { renderCheckList, runBaselineChecks } from '../phase2/baseline-checks.js';
import {
  loadPhase2ManualChecklist,
  renderPhase2ManualChecklist,
  savePhase2ManualChecklist,
} from '../phase2/manual-checklist.js';
...
const PATHS = {
  ...
  finalAudit: 'data/validation/audit-summary.json',
...
function renderSharedDebugFeed() {
  ...
  feed.innerHTML = `<div class="debug-entry-list">${entries
    .slice(-80)
    .reverse()
    .map((entry) => {
      const detail = escapeHtml(JSON.stringify(entry.detail ?? {}, null, 2));
      const error = entry.error
        ? `<p class="debug-hint"><b>Erro:</b> ${escapeHtml(entry.error.message || 'erro')}</p>`
        : '';
      const chipClass =
        entry.level === 'error'
          ? 'status-error'
...
async function renderPhase2Validation(companyId = state.selectedCompany) {
  ...
  if (!bundle) {
    autoChecks.innerHTML =
      '<p class="small-note">Abra a empresa selecionada para carregar os testes da Fase 2.</p>';
    summary.innerHTML =
      '<span class="status-chip status-warn">aguardando desbloqueio da empresa</span>';
    renderPhase2ManualChecklist(companyId);
    return;
  }

  const checks = runBaselineChecks(bundle, companyId);
  autoChecks.innerHTML = renderCheckList(checks);
...
function renderProofCards() {
  ...
  const html = [
    `<article class="card">
      ...
      <p class="metric-note">index.html, ${Array.from(
        document.querySelectorAll('link[rel="stylesheet"]')
      )
        .map((l) => l.getAttribute('href').split('/').pop())
        .join(', ')} e ${Array.from(document.querySelectorAll('script[src]'))
...
function renderCompanyPanel() {
  ...
  $('companyStatus').textContent =
    state.selectedCompany === 'empresa1' ? 'empresa 1 isolada' : 'empresa 2 isolada';
  $('companyStatus').className = 'status-chip status-ok';
  $('companyTitle').textContent =
    `${company.label} — ${companyShortDescription(state.selectedCompany)}`;
  ...
  $('companySources').innerHTML =
    ...
    <div class="source-item">
      ...
      .join('') || '<p class="small-note">Sem fontes listadas.</p>';

  $('datasetCards').innerHTML = coreFiles
    .map((file) => {
...
function renderDataQualityPanel() {
  ...
  const quality = getCompanyQuality();
  const required =
    quality.required_sources || company?.source_workbooks?.map((s) => s.filename) || [];
...
function renderPathAuditPanel() {
  ...
  const resultOk =
    missing.length === 0 && ['OK', true, 'FINAL_DEEP_AUDIT_OK'].includes(audit.result || true);
  const scenarioBlocks =
    state.proof?.empresa2_minimum_required_data?.core_tables?.scenario_blocks || 0;
  const checked =
    audit.path_count_checked || finalAudit.path_count_checked || finalAudit.checked_paths || 0;
  const workbooks =
    finalAudit.workbooks_checked ||
    finalAudit.workbook_count_checked ||
    finalAudit.xlsx_workbooks_checked ||
    0;
...
function renderSheetInventory() {
  ...
  if (!rows.length) {
    $('sheetInventorySummary').innerHTML =
      '<article class="card"><h3>Inventário indisponível</h3><p>Não foi possível carregar o CSV de abas.</p></article>';
    return;
  }
  ...
  const cards = Object.entries(byCompany)
    .map(
      ([company, values]) => `
    <article class="card">
  ...
  $('sheetInventoryTable').innerHTML = renderTable(
    rows,
    [
...
const manualChecks = [
  {
    id: 'empresa1_dados',
    label: 'Cliquei em Empresa 1 e vi demanda, distância e premissas',
    detail: 'Empresa 1 deve mostrar somente os datasets dela.',
  },
  {
    id: 'empresa2_dados',
    label: 'Cliquei em Empresa 2 e vi os datasets da planilha de malha',
    detail: 'Empresa 2 deve mostrar custos, estoque, faturamento, tributário e cenários.',
  },
  {
    id: 'sem_mistura_1',
    label: 'Confirmei que Empresa 1 não mostra dados da Empresa 2',
    detail: 'Não pode aparecer faturamento_uf, estoque ou dados_tributario na Empresa 1.',
  },
  {
    id: 'sem_mistura_2',
    label: 'Confirmei que Empresa 2 não mostra dados da Empresa 1',
    detail: 'Não pode aparecer demand_records ou distance_matrix como core da Empresa 2.',
  },
  {
    id: 'paths_zero',
    label: 'Conferi que não há paths faltantes',
    detail: 'O painel de auditoria deve mostrar 0 caminhos faltantes.',
  },
  {
    id: 'cenario_blocos',
    label: 'Conferi que Cenários da Empresa 2 aparece como bloco especial',
    detail: 'Deve existir scenario_blocks e scenario_totals nos datasets.',
  },
  {
    id: 'abas',
    label: 'Abri o inventário das abas e vi as planilhas mapeadas',
    detail: 'O resumo deve mostrar as abas inventariadas por empresa.',
  },
];
...
function renderManualChecklist() {
  ...
  $('manualChecklist').innerHTML = manualChecks
    .map(
      (check) => `
...
function updateManualProgress() {
  ...
  $('manualChecklistProgress').textContent =
    `${done}/${manualChecks.length} checagens manuais marcadas.`;
}
...
function runPhase1Checks() {
  ...
  const checks = [
    ...
    },
    {
      ...
      pass: ['index.html', 'assets/styles.css', 'assets/js/phase1/main.js'].every((p) =>
        htmlCssJs.includes(p)
      ),
    },
    {
  ...
  $('phase1AutoChecks').innerHTML = checks
    .map(
      (check) => `
  ...
  log('Testes automáticos da Fase 1 executados', {
    ...
    for (const target of targets) {
      try {
        ...
        const entry = (manifest.entries || []).find(
          (item) => item.original_path === normalizedTarget
        );
...
function renderPhaseRoadmap() {
  ...
  $('phaseRoadmap').innerHTML = Object.entries(tests)
    .map(([phase, config]) => {
...
async function ensureCompanyUnlocked(companyId) {
  const bundle = await loadPhase2Bundle(companyId);
...
function bindEvents() {
  ...
  $('selectEmpresa1').addEventListener('click', async () => {
    await ensureCompanyUnlocked('empresa1');
    setCompany('empresa1');
    window.location.hash = 'dados';
  });
  $('selectEmpresa2').addEventListener('click', async () => {
    await ensureCompanyUnlocked('empresa2');
    setCompany('empresa2');
    window.location.hash = 'dados';
  });
  $('runLivePathCheck').addEventListener('click', checkCorePathsLive);
  $('rerunPhase1Checks').addEventListener('click', runPhase1Checks);
  $('rerunBaselineChecks')?.addEventListener(
    'click',
  ...
  $('phase2ManualChecklist')?.addEventListener('change', (event) => {
    ...
    if (!id) return;
    const values = loadPhase2ManualChecklist(state.selectedCompany);
    values[id] = event.target.checked;
    savePhase2ManualChecklist(state.selectedCompany, values);
    renderPhase2ManualChecklist(state.selectedCompany);
  });
...
async function init() {
  try {
    ...
    const [catalog, proof, quality, finalAudit, pathAudit, phaseTests, workbookCsv] =
      await Promise.all([
    ...
    setTimeout(() => {
      ...
      if (
        protectedHashes.includes(currentHash) ||
        protectedHashes.some((ph) => currentHash.startsWith(ph))
      ) {
    ...
    document.body.insertAdjacentHTML(
      'beforeend',
      `<div class="card" style="margin:24px;color:#b42318"><h2>Erro ao carregar Fase 1</h2><pre>${escapeHtml(error.stack || error.message)}</pre></div>`
    );
  }
}
