export function selectFinalScenario({
  companyId,
  optimizerResult,
  ...
  if (!pool.length)
    return {
      company_id: companyId,
      selected_scenario: null,
      errors: ['nenhum cenário disponível para seleção'],
      warnings: [],
    };

  const candidates = pool.filter(
    (s) =>
      (s.company_id || s.scenario?.company_id || s.result?.company_id) === companyId ||
      !s.company_id
  );
  let selected = null;
  ...
  if (selectionMode === 'manual') {
    selected = candidates.find(
      (s) =>
        s.scenario_id === manualScenarioId ||
        s.scenario?.scenario_id === manualScenarioId ||
        s.result?.scenario_id === manualScenarioId
    );
    ...
    selected = [...candidates].sort(
      (a, b) =>
        Number(a.result?.total_with_tax ?? a.total_with_tax ?? Infinity) -
        Number(b.result?.total_with_tax ?? b.total_with_tax ?? Infinity)
    )[0];
    selection_reason = 'Cenário com menor total estimado.';
  } else if (selectionMode === 'best_by_robustness') {
    selected = [...candidates].sort(
      (a, b) =>
        Number(b.robustness?.robustness_score ?? b.quality?.quality_score ?? 0) -
        Number(a.robustness?.robustness_score ?? a.quality?.quality_score ?? 0)
    )[0];
  ...
  const warnings = [];
  if (usingLimitedPool)
    warnings.push('scored_scenarios ausente: seleção manual restrita ao top-10 (best_scenarios).');
  if (!selected)
    return {
      company_id: companyId,
      selected_scenario: null,
      errors: ['cenário selecionado não encontrado'],
      warnings,
    };
  if ((selected.quality?.risk_level || '').toLowerCase() === 'high')
    warnings.push('Cenário selecionado tem risco alto.');
  return {
    company_id: companyId,
    selected_scenario_id:
      selected.scenario_id || selected.scenario?.scenario_id || selected.result?.scenario_id,
