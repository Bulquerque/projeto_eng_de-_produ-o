...
export function evaluateScenarioQuality({ scenarioResult, baselineBundle, qualityRules = {} }) {
  ...
  const total = flows.reduce((a, f) => a + flowMeasure(f), 0);
  const byCd = {};
  flows.forEach((f) => {
    const cd = f.cd || 'SEM_CD';
    byCd[cd] = (byCd[cd] || 0) + flowMeasure(f);
  });
  const maxShare = Math.max(0, ...Object.values(byCd).map((v) => pct(v, total)));
  const reallocated = scenarioResult?.flow_summary?.reallocated_flows || 0;
  ...
  if (maxShare > maxCd) {
    alerts.push({
      severity: 'warning',
      type: 'concentration',
      message: `Maior concentração em CD: ${(maxShare * 100).toFixed(1)}%.`,
    });
    score -= Math.min(35, (maxShare - maxCd) * 100);
  }
  if (reallocShare > 0) {
    alerts.push({
      severity: reallocShare > maxRe ? 'warning' : 'info',
      type: 'reallocation',
      message: `${(reallocShare * 100).toFixed(1)}% dos fluxos foram realocados.`,
    });
    score -= Math.min(25, reallocShare * 30);
  }
  if (missing > maxMissing) {
    alerts.push({
      severity: 'error',
      type: 'coverage',
      message: `${(missing * 100).toFixed(1)}% dos fluxos ficaram sem atendimento.`,
    });
    score -= 40;
  }
  ...
  if (delta > 0.1) {
    alerts.push({
      severity: 'warning',
      type: 'cost',
      message: `Custo aumentou ${(delta * 100).toFixed(1)}% contra o baseline.`,
    });
    score -= 10;
  }
  ...
  return {
    ...
    risk_level: risk,
    quality_metrics: {
      max_cd_volume_share: maxShare,
      reallocated_flow_share: reallocShare,
