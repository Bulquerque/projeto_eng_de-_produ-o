...
function resolveTaxImpact({ scenario, baselineBundle, rebuilt, demandMultiplier }) {
  ...
  const sharedArgs = {
    baseTaxBlock: baseTax, flows: rebuilt.flows,
    scenario, baselineBundle, demandMultiplier,
  };

  if (taxMode === 'disabled') {
    return { taxImpact: 0, taxDetails: calculateReformTax({ ...sharedArgs, taxMode, taxRegime: 'disabled' }) };
  }
  ...
  const details     = calculateReformTax({ ...sharedArgs, taxMode: 'current', taxRegime: 'legacy_current' });
...
function calculateScenarioCosts({ companyId, scenario, baselineBundle, rebuilt }) {
  ...
  const { taxImpact, taxDetails } = resolveTaxImpact({ scenario, baselineBundle, rebuilt, demandMultiplier: dm });
...
export function runScenario({ companyId, scenario, baselineBundle }) {
  ...
  if (!validation.valid) {
    return {
      scenario_id: scenario?.scenario_id, company_id: companyId,
      simulation_status: 'invalid', flows: [], costs: {},
      tax_results: { total_tax_impact: 0 }, total_with_tax: null,
  ...
  return {
    ...
    validation,
    warnings: [...(validation.warnings || []), ...(rebuilt.warnings || []), ...(costs.physical_warnings || [])],
