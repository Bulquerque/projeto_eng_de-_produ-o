...
export async function loadScenarioLibrary(companyId) {
  ...
  let samples = [];
  try {
    samples = await loadPhase3Samples(companyId);
  } catch (e) {
    samples = [];
  }
  const baselineScenario = {
    ...
    base_scenario_id: baselineBundle.model.scenario_id,
  ...
  return {
    company_id: companyId,
    baselineBundle,
    scenarios: [baselineScenario, ...samples, ...saved],
    warnings: samples.length
      ? []
      : [{ message: 'Sem sample_scenarios.json; usando baseline e cenários salvos.' }],
    errors: [],
  };
}
