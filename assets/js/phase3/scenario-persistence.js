...
export function loadSavedScenarios(companyId) {
  try {
    return JSON.parse(localStorage.getItem(key(companyId)) || '[]');
  } catch {
    return [];
  }
}
export function saveScenario(companyId, scenario) {
  ...
  return { saved: true, storage_key: key(companyId), scenario_count: list.length };
}
export function deleteScenario(companyId, scenarioId) {
  ...
  localStorage.setItem(key(companyId), JSON.stringify(list));
  return { deleted: true, storage_key: key(companyId), scenario_count: list.length };
}
export function clearCompanyScenarios(companyId) {
  localStorage.removeItem(key(companyId));
  return { cleared: true, storage_key: key(companyId) };
}
