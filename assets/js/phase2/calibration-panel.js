...
function buildComparisonSource(fit, costs) {
  ...
  if (costs?.cost_basis === 'computed_proxy')
    return 'Baseline estrutural proxy calculado a partir de demanda, distância e premissas.';
...
export function renderCalibrationPanel(fit, costs = null) {
  ...
  const pendingNote = rows.length
    ? '<p class="small-note">Comparação real exibida a partir da referência canônica disponível; o Base Fit Score oficial segue pendente.</p>'
    : '<p class="benchmark-pending">Benchmark histórico pendente: existe baseline estrutural proxy, mas não há custo histórico consolidado para fechar o score oficial.</p>';
  return `<div class="fit-score-layout"><div class="fit-score-card ${statusClass(fit?.status)}"><span class="metric-label">Base Fit Score</span><strong class="fit-score-value">${escapeHtml(score)}</strong><span class="status-chip ${statusClass(fit?.status)}">${escapeHtml(fit?.status || 'sem status')}</span><p>${escapeHtml(source)}</p>${rows.length && fit?.status === 'benchmark_pending' ? '<p class="small-note">A comparação real está disponível, mas o score continua pendente por não haver benchmark independente.</p>' : ''}</div><div class="fit-table-wrap">${
    rows.length
      ? renderTable(
          rows,
          [
            { label: 'Métrica', value: 'metric' },
            { label: 'Referência', value: (r) => formatBRL(r.reference, true) },
            { label: 'Simulado', value: (r) => formatBRL(r.simulated, true) },
            {
              label: 'Erro',
              value: (r) => (r.percentage_error == null ? '—' : formatPct(r.percentage_error, 2)),
            },
