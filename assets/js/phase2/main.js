...
const state = {
  ...
  isLoading: false,
  logs: [],
  debug: createDebugSession({ phase: 'phase2', module: 'main', enabled: false }),
};
function log(msg, data = null) {
  state.debug.info(msg, data || {});
  appendSharedDebugEntry({
    phase: 'phase2',
    module: 'main',
    level: 'info',
  ...
  const line = `[${new Date().toLocaleTimeString('pt-BR')}] ${msg}${data ? `: ${JSON.stringify(data)}` : ''}`;
  state.logs.push(line);
  const el = $('phase2DebugConsole');
  if (el) el.textContent = state.logs.slice(-25).join('\n');
  const rich = $('phase2DebugStructured');
  if (rich) rich.innerHTML = renderDebugEntries(state.debug.entries);
}
function logError(msg, error, data = null) {
  state.debug.error(msg, error, data || {});
  appendSharedDebugEntry({
    phase: 'phase2',
    module: 'main',
    level: 'error',
  ...
  const rich = $('phase2DebugStructured');
  if (rich) rich.innerHTML = renderDebugEntries(state.debug.entries);
  const raw = $('phase2DebugConsole');
  if (raw)
    raw.textContent = state.logs
      .concat(`[erro] ${msg}: ${error?.message || error}`)
      .slice(-30)
      .join('\n');
}
...
async function loadCompany(companyId) {
  ...
  const loading = $('phase2Loading');
  const workspace = $('phase2Workspace');
  if (loading) loading.classList.remove('hidden');
  if (workspace) workspace.classList.add('hidden');
  try {
    if (!state.bundles[companyId]) state.bundles[companyId] = await loadPhase2Bundle(companyId);
    const bundle = state.bundles[companyId];
    const normalized = normalizePhase2Bundle(bundle, companyId);
    const baseline = buildBaselineScenario(normalized);
    const normalizedCheck = validateNormalizedInput(normalized);
    const baselineCheck = checkBaselineCompleteness(baseline);
    log('baseline:normalized', {
      companyId,
      valid: normalizedCheck.valid,
      baselineReady: baseline.baseline_ready,
    });
    if (!normalizedCheck.valid || !baselineCheck.valid) {
      state.debug.warn('baseline:validation-warning', { normalizedCheck, baselineCheck });
    }
    renderDashboard(bundle, companyId);
    renderTests(bundle, companyId);
    renderProof();
    ...
    log('Empresa carregada', {
      ...
      total: bundle.costs.costs.total_with_tax,
    });
  } catch (e) {
    if (loading) loading.classList.add('hidden');
    if (workspace) workspace.classList.remove('hidden');
    const warning = $('warningPanel');
    if (warning)
      warning.innerHTML = `<div class="alert-box error"><strong>Erro ao carregar o baseline</strong><p>${escapeHtml(e.message)}</p></div>`;
    logError('loadCompany:error', e, { companyId });
  } finally {
...
function companyContextCard(companyId) {
  ...
  const fitLabel =
    fit.base_fit_score !== null && fit.base_fit_score !== undefined
      ? `Base Fit ${fit.base_fit_score}`
      : String(rawFitStatus).includes('pending') || String(rawFitStatus).includes('pendente')
        ? 'pendente'
        : rawFitStatus;
...
function renderProof() {
  ...
  const pName = $('packageName');
  if (pName) pName.textContent = state.catalog?.package || 'pacote';
  const pVersion = $('packageVersion');
  if (pVersion) pVersion.textContent = state.catalog?.version || '—';
  const pStatus = $('phase2Status');
  if (pStatus)
    pStatus.textContent = r?.result === 'OK' ? 'Baseline pronto' : 'Baseline em validação';
  const pCards = $('phase2ProofCards');
  if (pCards)
    pCards.innerHTML = [companyContextCard('empresa1'), companyContextCard('empresa2')].join('');
}
function setup() {
  document
    .querySelectorAll('[data-company]')
    .forEach((b) => b.addEventListener('click', () => loadCompany(b.dataset.company)));
}
async function init() {
  try {
    setup();
